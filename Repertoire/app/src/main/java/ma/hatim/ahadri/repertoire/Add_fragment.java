...
public class Add_fragment extends DialogFragment
{
    private EditText first_name ;
    private EditText second_name ;
    private EditText email ;
    private EditText number ;
    private static  final String ADD = "add_contact" ;


    @Override
    public Dialog onCreateDialog(Bundle savedInstanceState)
    {
        View view = LayoutInflater.from(getActivity()).inflate(R.layout.add_layout, null);
        ...
        if(!first_name.getText().toString().isEmpty() && !second_name.getText().toString().isEmpty() && !email.getText().toString().isEmpty() && !number.getText().toString().isEmpty()) {
            {
                ...
                ContactDB.get(getActivity()).addContact(contact);
                Toast.makeText(getActivity(), "Contact cree", Toast.LENGTH_LONG).show();
